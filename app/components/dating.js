...
import {
  X, Heart, Star, MapPin, ShieldCheck, Zap, Flag, RefreshCw, Check, ArrowLeft,
...
const Avatar = ({ c, size = 56 }) => (
  <div className="rounded-full grid place-items-center text-white font-semibold shrink-0"
    style={{ width: size, height: size, background: c?.avatarColor || 'linear-gradient(135deg,#EF476F,#9B5DE5)', fontSize: size * 0.36 }}>
  ...
  return (
    <motion.div className="fixed inset-0 z-[70] flex flex-col bg-app-gradient" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <div className="flex items-center gap-3 p-4 pt-safe border-b border-border/60">
        ...
        <h1 className="font-display text-2xl flex items-center gap-2"><Heart size={20} className="text-rose-500" fill="#f43f5e" /> Rencontres</h1>
      </div>
          ...
          <div className="flex gap-1 p-1 m-4 rounded-2xl bg-muted/60">
            {[['discover', 'Découvrir'], ['likes', 'Likes'], ['matches', 'Matchs'], ['settings', 'Réglages']].map(([id, label]) => (
              <button key={id} onClick={() => setTab(id)}
                className={cx('press flex-1 py-2 rounded-xl text-xs font-medium', tab === id ? 'bg-card shadow text-foreground' : 'text-muted-foreground')}>{label}</button>
            ))}
          </div>
...
function Onboarding({ onDone }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto overscroll-contain px-4 pb-safe">
      <div className="rounded-2xl bg-rose-500/10 border border-rose-500/20 p-4 my-4 text-sm flex items-start gap-2">
      ...
      <input type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)}
        className="w-full mt-1 mb-4 rounded-2xl border border-border bg-card/60 px-4 py-3 text-sm outline-none focus:border-primary" />

      <label className="text-xs text-muted-foreground">Je suis</label>
      <div className="flex gap-2 mt-1 mb-4">
        {GENDERS.map(([v, l]) => (
          <button key={v} onClick={() => setGender(v)} className={cx('press flex-1 py-2.5 rounded-2xl text-sm font-medium border', gender === v ? 'bg-primary text-white border-primary' : 'bg-card/60 border-border')}>{l}</button>
        ))}
      </div>
      ...
      <div className="flex gap-2 mt-1 mb-4">
        {GENDERS.map(([v, l]) => (
          <button key={v} onClick={() => toggleSeek(v)} className={cx('press flex-1 py-2.5 rounded-2xl text-sm font-medium border', seeking.includes(v) ? 'bg-primary text-white border-primary' : 'bg-card/60 border-border')}>{l}</button>
        ))}
      </div>
      ...
      <div className="flex gap-2 flex-wrap mt-1 mb-4">
        {photos.map((p, i) => (
          <div key={i} className="relative">
            <img src={p} alt="" className="w-20 h-24 rounded-xl object-cover border border-border" />
        ...
        {photos.length < 6 && (
          <label className="w-20 h-24 rounded-xl border border-dashed border-border grid place-items-center cursor-pointer text-muted-foreground">
            <Camera size={20} />
      ...
      <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3} maxLength={500} placeholder="Parle un peu de toi…"
        className="w-full mt-1 mb-4 rounded-2xl border border-border bg-card/60 px-4 py-3 text-sm outline-none focus:border-primary resize-none" />

      <button onClick={locate} className="press w-full mb-3 rounded-2xl border border-border bg-card/60 px-4 py-3 text-sm flex items-center justify-center gap-2">
      ...
      <button onClick={submit} disabled={busy} className="press w-full rounded-2xl py-3.5 font-semibold text-white disabled:opacity-50 mb-6" style={{ background: 'linear-gradient(135deg,#EF476F,#9B5DE5)' }}>
    ...
    <div className="py-2">
      <div className="relative rounded-3xl overflow-hidden border border-border shadow-xl bg-card">
        <div className="relative aspect-[3/4] bg-muted">
          ...
          {current.boosted && <span className="absolute top-3 left-3 text-[11px] font-bold px-2.5 py-1 rounded-full text-white flex items-center gap-1" style={{ background: 'linear-gradient(135deg,#7a5b12,#E2AA2B)' }}><Zap size={11} /> Boosté</span>}
      ...
      <div className="flex items-center justify-center gap-5 mt-5">
        ...
        <button onClick={() => swipe('superlike')} disabled={busy} className="press w-16 h-16 rounded-full grid place-items-center text-white shadow-xl disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#4353F0,#2C39C7)' }}>
          <div className="flex flex-col items-center leading-none"><Star size={22} fill="#fff" /><span className="text-[9px] mt-0.5">15 ⚡</span></div>
        </button>
        <button onClick={() => swipe('like')} disabled={busy} className="press w-14 h-14 rounded-full grid place-items-center text-white shadow-lg disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#EF476F,#9B5DE5)' }}><Heart size={26} fill="#fff" /></button>
      </div>
      <p className="text-center text-[11px] text-muted-foreground mt-3">Super-like ⭐ = 15 Éclats · Like gratuit (limité/jour) · Position approximative</p>
      ...
      <AnimatePresence>
        {reporting && (
          <motion.div className="fixed inset-0 z-[80] grid place-items-center p-6 bg-black/50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setReporting(false)}>
            <div className="rounded-2xl bg-card border border-border p-5 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex gap-2">
                <button onClick={() => setReporting(false)} className="press flex-1 py-2.5 rounded-xl border border-border text-sm">Annuler</button>
                <button onClick={report} className="press flex-1 py-2.5 rounded-xl bg-destructive/10 text-destructive font-medium text-sm">Signaler</button>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {match && (
          <motion.div className="fixed inset-0 z-[80] grid place-items-center p-6" style={{ background: 'linear-gradient(160deg,rgba(239,71,111,.95),rgba(155,93,229,.95))' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="text-center text-white">
              ...
              <button onClick={() => { const c = match.conversationId; setMatch(null); onOpenConversation?.(c) }} className="press w-full max-w-xs mx-auto rounded-2xl py-3.5 font-semibold bg-white text-rose-600 flex items-center justify-center gap-2">
...
function LikesTab() {
  ...
  if (revealed) return (
    <div className="py-2 grid grid-cols-2 gap-3">
      ...
      {revealed.map((u) => (
        <div key={u.userId} className="rounded-2xl overflow-hidden border border-border">
          ...
          <div className="p-2"><div className="text-sm font-medium truncate">{u.name}{u.age ? `, ${u.age}` : ''} {u.superlike && <Star size={12} className="inline text-primary" fill="#4353F0" />}</div></div>
        </div>
      ))}
    </div>
  )
  return (
    <div className="text-center py-14">
      <div className="w-20 h-20 rounded-full mx-auto grid place-items-center mb-4" style={{ background: 'linear-gradient(135deg,#EF476F,#9B5DE5)' }}><Eye size={32} className="text-white" /></div>
      ...
      {data.count > 0 && (
        <button onClick={reveal} disabled={busy} className="press px-6 py-3 rounded-2xl font-semibold text-white disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#7a5b12,#E2AA2B)' }}>
          {busy ? <RefreshCw size={18} className="animate-spin" /> : `Révéler · ${data.revealCost} ⚡`}
        </button>
      )}
    </div>
  )
...
function MatchesTab({ onOpenConversation, onClose }) {
  ...
  return (
    <div className="py-2 space-y-2">
      {ms.map((m) => (
        <button key={m.userId} onClick={() => { onClose?.(); onOpenConversation?.(m.conversationId) }} className="press w-full flex items-center gap-3 p-3 rounded-2xl border border-border bg-card/60 text-left">
...
function SettingsTab({ profile, onChanged, onDeleted }) {
  ...
  const boost = async () => {
    ...
    setBusy('')
    setMsg(r.error ? r.error : `Profil boosté 🚀 · -${r.cost} ⚡`)
  }
  ...
  const del = async () => {
    if (!confirm('Supprimer ton profil Rencontres ? Tes matchs et likes seront effacés.')) return
    setBusy('del')
  ...
  return (
    <div className="py-2 space-y-3">
      {msg && <div className="rounded-2xl bg-gold/12 border border-gold/30 px-4 py-2.5 text-sm text-center">{msg}</div>}
      <button onClick={boost} disabled={busy === 'boost'} className="press w-full rounded-2xl px-4 py-4 font-semibold text-white flex items-center justify-center gap-2" style={{ background: 'linear-gradient(135deg,#7a5b12,#E2AA2B)' }}>
        {busy === 'boost' ? <RefreshCw size={18} className="animate-spin" /> : <><Zap size={18} /> Booster mon profil · 60 ⚡</>}
      </button>
      ...
      <button onClick={del} disabled={busy === 'del'} className="press w-full rounded-2xl px-4 py-3 font-medium border border-destructive/30 bg-destructive/10 text-destructive flex items-center justify-center gap-2">
