...
import { X, Zap, Trophy, RefreshCw, Info, ChevronRight, Star } from 'lucide-react'
  ...
  return (
    <motion.div className="fixed inset-0 z-[70] flex flex-col bg-app-gradient" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <div className="flex items-center gap-3 p-4 pt-safe border-b border-border/60">
        ...
        {home && <span className="ml-auto text-sm font-display tabular">{home.balance} ⚡</span>}
      </div>
...
function Home({ home, onPlay, busy, err }) {
  ...
  return (
    <div className="py-4 space-y-4">
      <div className="rounded-2xl bg-primary/8 border border-primary/20 p-3 text-xs text-muted-foreground flex items-start gap-2">
      ...
      <div className="rounded-[var(--radius)] overflow-hidden border border-border">
        <div className="p-5 text-white relative" style={{ background: 'linear-gradient(135deg,#3a2f0e,#7a5b12 55%,#E2AA2B)' }}>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {reflex.rewards.map((t) => (
              <span key={t.scoreMin} className="text-[11px] px-2 py-1 rounded-full bg-muted/60">score ≥ {t.scoreMin} → +{t.eclats} ⚡</span>
            ))}
          </div>
          {err && <p className="text-xs text-destructive">{err}</p>}
          <button onClick={onPlay} disabled={busy} className="press w-full rounded-2xl py-3.5 font-semibold text-white disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#7a5b12,#E2AA2B)' }}>
            {busy ? <RefreshCw size={18} className="animate-spin mx-auto" /> : (reflex.freeLeft > 0 ? 'Jouer (gratuit aujourd\'hui)' : `Jouer · ${reflex.entryCost} ⚡`)}
          </button>
        </div>
      </div>

      {/* Bientôt : Mémoire */}
      <div className="rounded-2xl border border-border bg-card/40 p-4 flex items-center gap-3 opacity-70">
...
function ReflexGame({ session, onEnd }) {
  ...
  if (!started) return (
    <div className="py-16 text-center">
      ...
      <button onClick={() => setStarted(true)} className="press px-8 py-3.5 rounded-2xl font-semibold text-white" style={{ background: 'linear-gradient(135deg,#7a5b12,#E2AA2B)' }}>Commencer</button>
    </div>
  )
  ...
  return (
    <div className="py-3">
      ...
      <div className="relative w-full rounded-3xl border border-border overflow-hidden select-none" style={{ aspectRatio: '3/4', background: 'radial-gradient(circle at 50% 30%, rgba(226,170,43,0.14), transparent 60%)' }}>
        <motion.button key={`${pos.x}-${pos.y}`} onClick={hit} aria-label="Toucher"
          ...
          className="absolute w-16 h-16 rounded-full grid place-items-center text-white shadow-xl"
          style={{ left: `${pos.x}%`, top: `${pos.y}%`, background: 'linear-gradient(135deg,#F0CE7E,#E2AA2B,#B98514)' }}>
...
function Result({ result, onAgain }) {
  if (result?.error) return (
    <div className="py-16 text-center">
      ...
      <button onClick={onAgain} className="press px-6 py-3 rounded-2xl bg-primary/10 text-primary font-medium">Retour</button>
    </div>
  )
  return (
    <div className="py-14 text-center">
      ...
      <div className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-white font-semibold" style={{ background: result.reward > 0 ? 'linear-gradient(135deg,#7a5b12,#E2AA2B)' : '#6b7280' }}>
        {result.reward > 0 ? <><Star size={16} fill="#fff" /> +{result.reward} Éclats</> : 'Aucune récompense cette fois'}
      </div>
      <div className="text-sm text-muted-foreground mt-3">Meilleur score : {result.myBest} · Solde : {result.balance} ⚡</div>
      <button onClick={onAgain} className="press mt-6 px-8 py-3.5 rounded-2xl font-semibold text-white" style={{ background: 'linear-gradient(135deg,#7a5b12,#E2AA2B)' }}>Rejouer</button>
    </div>
  )
}
