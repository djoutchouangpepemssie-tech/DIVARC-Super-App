...
function Discover({ onTab, onOpenDating, onOpenArcade, onOpenNetwork }) {
      ...
      <div className="cascade space-y-5">
        ...
        <button onClick={onOpenNetwork} className="press w-full text-left rounded-[var(--radius)] p-5 relative overflow-hidden" style={{ background: 'linear-gradient(135deg,#4353F0,#2C39C7)' }}>
          <div className="relative flex items-center gap-4 text-white">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-display text-2xl leading-tight">Réseau <span className="text-xs align-top opacity-80">bêta</span></div>
              <div className="text-sm text-white/85">Publie, suis, partage · confiance-first</div>
            </div>
            <ChevronRight size={22} className="text-white/80" />
          </div>
        </button>
  ...
  return (
    <div className="font-body text-foreground">
      ...
      <AnimatePresence>
        ...
        {overlay === 'network' && <NetworkModule me={user} onClose={() => setOverlay(null)} />}
      </AnimatePresence>
