...
backend:
  ...
  - task: "Ads Manager v2 (Google Ads-like): config, estimate, keywords, campaigns, insights, tracking [PHASE9]"
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "MAJOR rewrite of Ads Manager. Campaign schema v2: type(search/display/video/shopping), objective(sales/leads/traffic/awareness/app), budgetType(daily/total), dailyBudgetCents, bidStrategy(cpc/cpm/maximize/target_cpa), maxBidCents, targeting{locations,radiusKm,ageRange,genders,interests,devices}, keywords[{text,matchType,bidCents}], creative{headline,headline2,body,cta,emoji,mediaUrl,priceCents,finalUrl}, metrics(impressions,clicks,spentCents,conversions), daily[] time-series. On CREATE: charges budgetCents from wallet (402 if insufficient), generates simulateHistory (7 days) so analytics are populated immediately (spentCents capped at budget), creates 'Publicité' transaction. NEW ENDPOINTS: GET /ads/config (types/objectives/bidStrategies/interests/devices/ageRanges/genders); GET /ads/keywords?q= (10 keyword suggestions w/ volume,competition,suggestedBidCents); POST /ads/estimate {dailyBudgetCents,bidStrategy,maxBidCents,targeting} -> reach estimate {audience,impressionsPerDay[min,max],clicksPerDay,reachPerDay,estCpcCents,estCtr}; GET /ads/insights -> {totals(impr,clicks,spent,conv,ctr,cpc,convRate),daily[14d aggregated],top[5],counts{total,active,paused}}; POST /ads/campaigns (create); GET /ads/campaigns (list w/ adDerived: ctr,cpcCents,cpmCents,convRate,cpaCents,remainingCents); GET /ads/campaigns/:id (detail); PATCH /ads/campaigns/:id (status pause/active/ended[refund remaining to wallet] + edit name/maxBid/bidStrategy/dailyBudget/targeting/creative/keywords); DELETE /ads/campaigns/:id (refund remaining if not ended); POST /ads/campaigns/:id/track {type:impression|click|conversion} (cost: click=maxBidCents, impression=cpm/1000, conversion=0; updates today's daily bucket; auto-ends when spent>=budget). Sponsored feed injection (getSponsored) still works with creative fields (unchanged). All require Bearer. NOTE: old campaigns cleared from DB."
        -working: true
        -agent: "testing"
...
test_plan:
  current_focus:
    - "Ads Manager v2 (Google Ads-like): config, estimate, keywords, campaigns, insights, tracking [PHASE9]"
  stuck_tasks: []
  test_all: false
